...
# ── Module-level shared fixtures (no assertions here) ─────────────────────────
...
def test_cpp_accessor_compat():
    ...
    for c in range(cpp.nlist):
        ...
        assert _U.shape == (cpp.d, cpp.F), f"get_U_drift({c}) wrong shape: {_U.shape}"
        assert _U.dtype == np.float32,     f"get_U_drift({c}) wrong dtype: {_U.dtype}"
        for f in range(cpp.F):
            cone = cpp.get_cone(c, f)
            _ = cone.size()
            _ = cone.all_ids()
            _projs, _ids = cone.get_axis_pairs(0)
            assert _projs.dtype == np.float32, f"get_axis_pairs projs wrong dtype: {_projs.dtype}"
            assert _ids.dtype == np.uint32,    f"get_axis_pairs ids wrong dtype: {_ids.dtype}"
            assert _projs.shape == _ids.shape, "get_axis_pairs projs/ids shape mismatch"
    ...
    cpp.set_merge_params(0, 1.0, 0.5)

    _idx_cos_compat = AMPIAffineFanIndex(data, nlist=5, num_fans=16, seed=0, metric='cosine')
...
def test_binary_query_candidates():
    _q0 = qs[0]
...
def test_add_delete_update_api():
    ...
    _, _, _ids_pre = _idx3b.query(_q3, k=10, window_size=200,
                                   probes=_idx3b.nlist, fan_probes=_idx3b.F)
    assert _gid_del in _ids_pre.tolist(), "inserted point not found before delete"
    _idx3b.delete(_gid_del)
    _, _, _ids_post = _idx3b.query(_q3, k=10, window_size=200,
                                    probes=_idx3b.nlist, fan_probes=_idx3b.F)
...
def test_mmap_backed_data_buffer():
    with tempfile.TemporaryDirectory() as _mmap_dir:
        ...
        if _HAS_CPP:
            ...
            _expected_bytes = _mmap_idx._cpp.capacity * 32 * 4
